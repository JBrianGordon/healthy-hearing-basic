				...
				<div class="caCallGroups index content">
					...
					<div class = "btn-toolbar">
						<?= $this->Form->create(null, [
							...
							<?= $this->Form->hidden('status', ['id' => 'CaCallGroupStatus']) ?>
							<?= $this->Form->hidden('score', ['id' => 'CaCallGroupScore']) ?>
							<?= $this->Form->hidden('is_appt_request_form', ['id' => 'CaCallGroupIsApptRequestForm']) ?>
							<?php if ($isCallSupervisor): ?>
								<button type="submit" class="btn btn-default btn-xs" id="followupNoAnswerBtn"><i class="bi bi-circle-fill orange"></i> Supervisor</button>
								<hr class='mt5 mb10'>
							<?php endif; ?>
							<button type="submit" class="btn btn-default btn-xs" id="vmCallbackBtn"><i class="bi bi-circle-fill pink"></i> Voicemails</button>
							...
							<button type="submit" class="btn btn-default btn-xs" id="clinicFollowupBtn"><i class="bi bi-circle-fill yellow"></i> Followup - Set Appt</button>
							<button type="submit" class="btn btn-default btn-xs" id="followupTentativeBtn"><i class="bi bi-circle-fill ivory"></i> Followup - Verify Tentative Appt</button>
				            ...
				            <tbody>
				                <?php foreach ($caCallGroups as $caCallGroup): ?>
				                    ...
				                    <tr class='<?= $class ?>'>
				                        ...
				                        <td><?= $readableCallType; ?></td>
                                        <td>
                                            <?= date('M d,', strtotime($caCallGroup->ca_calls[0]->start_time)); ?><br>
                                            <?= date('g:i a', strtotime($caCallGroup->ca_calls[0]->start_time)); ?>
                                        </td>
                                        <td>
                                            <?= date('M d,', strtotime($caCallGroup->scheduled_call_date)); ?><br>
                                            <?= date('g:i a', strtotime($caCallGroup->scheduled_call_date)); ?>
                                        </td>
				                        <td>
                                            <?php
                                            if (!empty($caCallGroup->location_id)) {
                                                if (!empty($caCallGroup->location->title)) {
                                                    ...
                                                    echo '<span class="badge label-default">'.$this->Clinic->getClinicTimezoneAbbr($caCallGroup->location_id).'</span>';
                                                } else {
				                        ...
				                        <td>
                                            <?= $caCallGroup->caller_first_name.' '.$caCallGroup->caller_last_name ?><br>
                                            <?= $caCallGroup->patient_first_name.' '.$caCallGroup->patient_last_name ?>
                                        </td>
				                        <td class="p5" nowrap>
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php endif; ?>
                                                <?php if ($showCallBtn): ?>
                                                    <?= $this->Html->link('Lock and Call', ['controller'=>'ca_calls', 'action' => 'add_outbound', $caCallGroup->id], ['class' => 'btn btn-default']) ?>
                                                <?php endif; ?>
                                            <?php endif; ?>
				                        </td>
				                    </tr>
