...
?>
<?php
/*** TODO: uncomment once Provider is pulled in ***
...
<div class="col-md-12 page-content">
			...
			<?php if ($location->is_call_assist/* && $isEnhancedOrPremier*/): ?>
				...
				<div id="apptRequestModal">
						<div class="modal-content">
							<?php echo $this->B3F->create('CaCall', [
								...
								'class' => 'form-horizontal apptRequestForm',
									...
									<?php
									echo $this->B3F->hidden('CaCallGroup.location_id', ['value'=>$location['Location']['id']]);
									echo $this->B3F->hidden('CaCallGroup.is_patient', ['value'=>true]);
									echo $this->B3F->hidden('CaCallGroup.is_appt_request_form', ['value'=>true]);
									echo $this->B3F->hidden('CaCallGroup.traffic_source', ['value'=>'unknown']);
									echo $this->B3F->hidden('CaCallGroup.traffic_medium', ['value'=>'unknown']);
									?>
									<div class='form-fields col-xs-12'>
										<?php
											echo $this->B3F->input('CaCallGroup.caller_first_name', [
											...
											echo $this->B3F->input('CaCallGroup.caller_last_name', [
											...
											echo $this->B3F->input('CaCallGroup.caller_phone', [
												'label' => 'Phone number:',
											...
											echo $this->B3F->input('CaCallGroup.email', [
												'type' => 'email',
												'label' => 'Email:',
											...
											<div class="col col-md-7">
												<div class="checkbox">
													...
													foreach ($topicOptions as $topicKey => $label) {
														echo $this->B3F->input('CaCallGroup.'.$topicKey, [
															'label' => [
																'class' => 'control-label pt0',
																'style' => 'text-align:left;',
																'text' => '<span class="topic-label vwo-test-old-label">' . $label . '</span>',
															],
															'wrapInput' => false,
															'div' => false,
															'class' => false,
									...
									<p class="col-xs-12"><small class="tac help-block"><?php echo Configure::read('siteName'); ?> will contact you regarding your request as soon as possible.</small></p>
									<div class="row col-xs-12 ml0">
										...
										<div class="g-recaptcha"
											 data-sitekey="<?php echo Configure::read('recaptchaPublicKey'); ?>"
					...
					<div>
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4>Thank you for requesting an appointment at <?php echo $location['Location']['title']; ?></h4>
							</div>
							<div class="modal-body">
								<p class="lead">
									<?php echo Configure::read('siteName'); ?> will contact you as soon as possible to assist you.<br><br>
	...
	<div class="row" style="clear: both">
		...
		<div class="col-md-8">
			...
			<?php if(false/*!$this->App->isMobileDevice()*/){
		...
		<div class="col-md-4">
			...
			<?php if($location->is_cq_premier && isset($location['LocationVidscrips'][0]['LocationVidscrips']['vidscrip'])) : ?>
				<div class="vidscrip-container" style="margin: 0 auto 20px; max-width: 600px">
					<div id="vidscrip-embed-<?php echo $location['LocationVidscrips'][0]['LocationVidscrips']['vidscrip']; ?>"></div>
								...
								var vidscripScroll = window.addEventListener("scroll",function() {
								    if (vidscripScript[0].getAttribute("src") == null && isInViewport(vidscripContainer[0])) {
									    ...
									    if(typeof(addVidscripWidget) !== 'undefined') {
								        	addVidscripWidget({ elementId: "vidscrip-embed-<?php echo $location['LocationVidscrips'][0]['LocationVidscrips']['vidscrip']; ?>", vidscrips: ["<?php echo $location['LocationVidscrips'][0]['LocationVidscrips']['vidscrip']; ?>"], theme: "v2", email: "<?php echo $location['LocationVidscrips'][0]['LocationVidscrips']['email']; ?>" });
					...
					<div class="panel panel-light" id="specialAnnouncement">
						...
						<div class="panel-body">
							<div class="panel-section condensed">
								...
								?>
								<img loading="lazy" width="300" height="300" src="<?php echo '/img/coupons/coupon-'.$location['Location']['coupon_id'].'.jpg'; ?>" alt="<?php echo $couponAlt; ?>">
							</div>
						</div>
				...
				<?php elseif (!empty($location['LocationAd'])): ?>
					<div class="panel panel-light" id="specialAnnouncement">
						<?php if (!empty($location['LocationAd'][0]['LocationAd']['title'])): ?>
							<header class="panel-heading text-center">
								<h2><?php echo $location['LocationAd'][0]['LocationAd']['title']; ?></h2>
							</header>
						<?php endif; ?>
						<?php if (!empty($location['LocationAd'][0]['LocationAd']['photo_url'])): ?>
							<div class="panel-body">
								<div class="panel-section condensed">
									<img loading="lazy"<?php if(!empty($location['LocationAd'][0]['LocationAd']['border'])){ echo ' class="' . $location['LocationAd'][0]['LocationAd']['border'] . '"';} ?> width="300" height="300" src="<?php echo '/cloudfiles/clinics/' . $location['LocationAd'][0]['LocationAd']['photo_url']; ?>" alt="Announcement for <?php echo $location['Location']['title'] ?>">
								</div>
							</div>
						<?php endif; ?>
						<?php if (!empty($location['LocationAd'][0]['LocationAd']['description'])): ?>
							<footer class="panel-footer tac">
								<?php echo $location['LocationAd'][0]['LocationAd']['description']; ?>
							</footer>
						<?php endif; ?>
					</div>
				<?php endif; ?>
...
<?php endif; ?>
<?php if (in_array($location->direct_book_type, [Location::DIRECT_BOOK_BLUEPRINT, Location::DIRECT_BOOK_EARQ]) && !empty($location->direct_book_iframe)): ?>
	<?php echo $this->element('locations/profile/direct_book_modal', ['iframe' => $location['Location']['direct_book_iframe'], 'locationId' => $location->id, 'locationTitle' => $location->title]); ?>
<?php endif; ?>