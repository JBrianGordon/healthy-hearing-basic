...
<p><?= $this->Html->url($hhUrl, true) ?></p>

<?php if ($isUS): ?>
<p><?= $siteName ?> is the internet's leading source of consumer information about hearing loss and hearing aids, with millions and millions of visitors a year. It’s also a valuable lead-generating resource for hearing clinics; we send consumer telephone calls to hearing clinics listed in our directory, resulting in thousands of appointments set and hearing aids sold each year.</p>
<?php endif ?>

<p>To maximize the referral-generating potential of this <?= $siteName ?> profile for your clinic, please ensure that the profile is completed and kept up to date so consumers can find your clinic online.</p>

<p><?php if ($isUS){echo '<strong>';}?>To log in and edit the profile for this location, please <?= $this->Html->link('click here', $this->Html->url('/clinic', true)) ?>. Your <?= $siteName ?> account number is <?php if (!$isUS){echo '<strong>';}?><?= $clinicUsername ?><?php if (!$isUS){echo '</strong>';}?>. Your <?= $siteName ?> password is <?php if (!$isUS){echo '<strong>';}?><?= $defaultPassword ?><?php if (!$isUS){echo '</strong>';}?> and it is case-sensitive. Please keep this username and password someplace handy so you can log back in later and check on your calls and reviews, as well as make updates to the profile as necessary.<?php if ($isUS){echo '</strong>';}?></p>
...
<p>After you’re logged into this profile, please visit our <?= $this->Html->link('Frequently Asked Questions', $this->Html->url('/clinic/pages/faq', true)) ?> section to learn more about how this <?= $siteName ?> profile works to benefit your practice.</p>
