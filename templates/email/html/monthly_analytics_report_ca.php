...
Here’s a quick snapshot of the activity on the Hearing Directory profile for <?= $clinicTitle ?> at <?= $clinicAddress ?>:<br><br>
 
<strong>Last month (<?= date("F", strtotime("last month")) ?>) &ndash;</strong><br>
This Hearing Directory profile was visited <strong><?= $profileViewsForMonth ?></strong> 
<?= simplePluralize(['time'], $profileViewsForMonth) ?>.<br>
<?php if ($callsForMonth['all_calls']['total'] > 0): ?>
	The unique tracking phone number for this Hearing Directory profile received <strong><?= $callsForMonth['all_calls']['total'] ?></strong> <?= simplePluralize(['call'], $callsForMonth['all_calls']['total']) ?>.<br>
...
<?php endif ?>
This Hearing Directory profile was visited <strong><?= $profileViewsYTD ?></strong> 
<?= simplePluralize(['time'], $profileViewsYTD) ?>.<br>
<?php if ($callsYTD['all_calls']['total'] > 0): ?>
	The unique tracking phone number for this Hearing Directory profile received <strong><?= $callsYTD['all_calls']['total'] ?></strong> <?= simplePluralize(['call'], $callsYTD['all_calls']['total']) ?>.<br>
...
<?php else: ?>
	reviews.
<?php endif ?> For ideas about how to get more reviews, please <?= $this->Html->link('contact us', 'mailto:contacthd@hearingdirectory.com?subject=Hearing Directory profile reviews - ' . $clinicUsername) ?>.<br><br>
...
If you have any questions or need assistance with this Hearing Directory profile, please contact us at <?= $this->Html->link('contacthd@hearingdirectory.com', 'mailto:contacthd@hearingdirectory.com?subject=Hearing Directory profile inquiry - ' . $clinicUsername) ?>.<br><br>
 
Thanks for partnering with Hearing Directory!<br>
<?php
	echo $this->Html->image(
		Configure::read('logo'), [
			'alt' => 'Hearing Directory logo',
			'fullBase' => true,
			'width' => '200px'
		]
	);
