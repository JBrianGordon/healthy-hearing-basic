...
<div class="well provider" provider="<?php echo $key; ?>">
	...
	<?php if (Configure::read('showProviderCredentialButtons')): ?>
		...
		<?php
			echo '<div class="form-group"><div class="col col-md-9 col-md-offset-3 mb10 p0">';
				echo $this->Form->control("Provider." . $key . ".credentials", ['label' => false, 'wrapInput' => 'col col-md-9 col-md-offset-3', 'value'=>$provider->credentials ?? '']); 
			echo '</div></div>';
		?>
	<?php else: ?>
	...
	<?php
		if (isset($provider)) {
			...
			//echo "<div class='form-group'><div class='col col-md-9 col-md-offset-3'>" . $this->Clinic->providerImage($provider) . "</div></div>";
		}
		if (function_exists("imagecreate")) {
			echo $this->Form->control("Provider." . $key . ".file", ['type' => 'file', 'label' => ['text' => 'Upload Image', 'class' => 'col-sm-3 control-label'], 'value' => $provider->image_url ?? '', 'class' => 'col-sm-9']);
