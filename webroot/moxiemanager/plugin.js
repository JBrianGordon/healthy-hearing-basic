		...
		return function(settings) {
			...
			if (!moxman.ui) {
				...
				ResourceLoader.load({
					...
					if (typeof settings.with_credentials === 'boolean') {
						Env.withCredentials = settings.with_credentials;
						moxman.Env.withCredentials = settings.with_credentials;
					}
...
define("moxman/interop/TinyMcePlugin", [
	...
	function registerOption(editor, name, value) {
		if (editor.options) {
			// V6 API
	...
	function getOption(editor, name) {
		// Legacy call we can't use editor.options.get since it requires us to register the option
	...
	tinymce.PluginManager.add('moxiemanager', function(editor, url) {
		...
		Env.language = getOption(editor, 'language');
		Env.cacheSuffix = getOption(editor, 'cache_suffix');
		...
		function getBrowseSettings(type) {
			...
			var availableSettings = [
				...
				'editimage_main_toolbar',
				'editimage_zoom_toolbar',
				...
				'filelist_insert_event',
				'filelist_insert_toolbar',
				'filelist_main_toolbar',
				'filelist_manage_menu',
				'filelist_utils_toolbar',
				'filter',
				'fullscreen',
				'height',
				'hidden_tools',
			...
			tinymce.each(availableSettings, function(key) {
				browseSettings[key] = getOption(editor, 'moxiemanager_' + key);
			});

			// Extend with type specifc settings and remove any prefix
			var typeSettings = getOption(editor, 'moxiemanager_' + type + '_settings');
			if (typeSettings) {
		...
		function browse(value, type, callback) {
			...
			Loader.browse(tinymce.extend({
				zIndex: zIndex,
				url: url,
				document_base_url: getOption(editor, 'document_base_url'),
		...
		if (tinyMCEVersion >= 5) {
			registerOption(editor, 'file_picker_callback', file_picker_callback);
		} else {
			registerOption(editor, 'file_browser_callback', file_browser_callback);
		}
