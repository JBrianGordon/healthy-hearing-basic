...
class MOXMAN_Vfs_FileList implements IteratorAggregate {
	...
	public function getFiles() {
		...
		if ($orderBy == 'name') {
			usort($dirs, array($this, 'compareNames'));

			if ($desc) {
				$dirs = array_reverse($dirs);
			}

			usort($files, array($this, 'compareNames'));
		} elseif ($orderBy == 'extension') {
	...
	private function compareNames($a, $b) {
		$aName = pathinfo($a[1], PATHINFO_FILENAME);
		$bName = pathinfo($b[1], PATHINFO_FILENAME);
		if (ctype_digit($aName) && ctype_digit($aName)) {
			$aInt = (int) $aName;
			$bInt = (int) $bName;
			if ($aInt === $bInt) {
				return 0;
