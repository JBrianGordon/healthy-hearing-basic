...
class MOXMAN_Vfs_Local_File extends MOXMAN_Vfs_BaseFile {
	...
	private function needsWindows1252Encoding() {
		// It seems that PHP version 7.0 added support for UTF-8 encoding.
		// It will now automatically map to Windows-1252 or UTF-8 encoding depending on OS and locale settings.
		// But we are keeping this in case that isn't the case for some OS.
		return false;
	}

	private function fromUtf($path) {
		if (DIRECTORY_SEPARATOR == "\\" && function_exists("mb_convert_encoding")) {
			$path = $this->needsWindows1252Encoding() ? mb_convert_encoding($path, "Windows-1252", "UTF-8") : $path;
	...
	private function toUtf($path) {
		...
		if (DIRECTORY_SEPARATOR == "\\" && function_exists("mb_convert_encoding") && $this->needsWindows1252Encoding()) {
