...
init = () => {
  ...
  const textLength = document.querySelectorAll('td.body:contains("yhn")').length;
  const importTextLength = document.querySelectorAll('u:contains("YHN Import")').length;

  if (document.querySelector('.navbar-logo.CA') && (textLength > 0 || importTextLength > 0)) {
    if (textLength > 0) {
      const bodyTdElements = document.querySelectorAll('td.body:contains("yhn")');
      bodyTdElements.forEach(element => {
        element.innerHTML = element.innerHTML.replace(/yhn[_]?/g, '');
      });
    }
    if (importTextLength > 0) {
      const importElements = document.querySelectorAll('u:contains("YHN Import")');
  ...
  document.querySelector('#LocationDirectBookType').addEventListener('change', function() {
    editObj.onChangeDirectBookType(this.value);
  });
  document.querySelector('#LocationDirectBookType').dispatchEvent(new Event('change'));

  document.querySelector('#LocationIsListingTypeFrozen').addEventListener('change', function() {
    editObj.onChangeIsListingTypeFrozen(this.checked);
  });
  document.querySelector('#LocationIsListingTypeFrozen').dispatchEvent(new Event('change'));

  document.querySelector('#LocationFeatureContentLibrary').addEventListener('change', function() {
    editObj.onChangeFeatureContentLibrary(this.checked);
  });
  document.querySelector('#LocationFeatureContentLibrary').dispatchEvent(new Event('change'));

  document.querySelector('#LocationFeatureSpecialAnnouncement').addEventListener('change', function() {
    editObj.onChangeFeatureSpecialAnnouncement(this.checked);
  });
  document.querySelector('#LocationFeatureSpecialAnnouncement').dispatchEvent(new Event('change'));

  document.querySelector('#LocationHourIsClosedLunch').addEventListener('change', function() {
    editObj.onChangeIsClosedLunch(this.checked);
  });
  document.querySelector('#LocationHourIsClosedLunch').dispatchEvent(new Event('change'));

  document.querySelector('#LocationIsMobile').addEventListener('change', function() {
    editObj.onChangeIsMobile(this.checked);
  });
  document.querySelector('#LocationIsMobile').dispatchEvent(new Event('change'));

  editObj.locationAutocomplete();
  editObj.initSpecialAnnouncements();
}

export function selectImport(importId) {
...
export function selectCqpImport(cqpImportId) {
...
export function addVideoRow(obj) {
  const editObj = this;
...
export function removeVideoRow(obj) {
  const row = obj.closest('tr');
...
export function addLink(locationId, key) {
  const editObj = this;
...
export function deleteLink(obj) {
  const key = obj.dataset.key;
...
export function onChangeDirectBookType(directBookType) {
...
export function onChangeFeature(isFeature, elementId, requiredElementId) {
...
export function scrollTo(selector, offset = 90) {
...
export function chooseOwnCoupon() {
  const couponLibrary = document.querySelector('#couponLibrary');
  const couponSelected = document.querySelector('#couponSelected');
  const uploadCoupon = document.querySelector('#uploadCoupon');
...
export function showCouponLibrary() {
  const couponLibrary = document.querySelector('#couponLibrary');
  const couponSelected = document.querySelector('#couponSelected');
  const uploadCoupon = document.querySelector('#uploadCoupon');
...
export function onClickHoursClosed(obj) {
...
export function onChangeIsClosedLunch(isClosedLunch) {
  if (isClosedLunch) {
...
const locationIsEarq4y = document.querySelector("#LocationIsEarq4y");
const providerCheckboxes = document.querySelectorAll(".provider .checkbox label input");

locationIsEarq4y.insertAdjacentHTML("afterend", "<span class='slider' style='margin-left:265px'></span>");

providerCheckboxes.forEach((checkbox) => {
  checkbox.parentNode.insertAdjacentHTML("afterend", "<span class='slider' style='margin-left:265px'></span>");
});
...
const locationIsIdaVerified = document.querySelector("#LocationIsIdaVerified");

insertAfter(locationIsIdaVerified, "<span class='slider' style='margin-left:235px'></span>");
