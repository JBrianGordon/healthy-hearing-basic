...
class AlterTableEngines extends AbstractMigration
{
    ...
    {
        // Take care of invalid dates in Locations table
        $invalidDates = $this->execute('SET SQL_MODE=\'ALLOW_INVALID_DATES\'');
        $builder = $this->getQueryBuilder();
        $builder
            ->update('locations')
            ->set('last_edit_by_owner_date', NULL)
            ->where(['last_edit_by_owner_date' => '0000-00-00 00:00:00'])
            ->execute();

        // Change all table engines to InnoDB
        $tables = ['advertisements', 'call_sources', 'ca_call_group_notes', 'ca_call_groups', 'ca_calls', 'call_sources', 'cities', 'configurations', 'content', 'content_tags', 'content_users', 'corps', 'corps_users', 'crm_searches', 'cs_calls', 'drafts', 'icing_versions', 'import_status', 'location_ads', 'location_emails', 'location_hours', 'location_notes', 'location_photos', 'location_user_logins', 'location_users', 'location_videos', 'location_vidscrips', 'locations', 'providers', 'queue_task_logs', 'queue_tasks', 'quiz_results', 'reviewers_wikis', 'reviews', 'schema_migrations', 'seo_blacklists', 'seo_canonicals', 'seo_honeypot_visits', 'seo_meta_tags', 'seo_redirects', 'seo_search_terms', 'seo_status_codes', 'seo_titles', 'seo_uris', 'sitemap_urls', 'tag_ads', 'tag_wikis', 'tags', 'users', 'users_wikis', 'wikis', 'zipcodes'];
